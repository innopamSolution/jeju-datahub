...
export function thumbHtml(it, catMap, gallery) {
  ...
  if (it.cat === 'pointcloud') {
    const realAttrs = it.pointCloudUrl ? ` class="sams-tt sams-tt-real" data-url="${it.pointCloudUrl}"` : ` class="sams-tt" data-color="${c.color}"`;
    return `<div style="position:relative;height:138px;background:radial-gradient(circle at 50% 40%, #14202e, #060a10);"><canvas${realAttrs} width="248" height="138" style="width:100%;height:138px;display:block;"></canvas><div style="position:absolute;left:9px;bottom:8px;display:flex;align-items:center;gap:5px;font-size:10px;color:rgba(255,255,255,0.8);background:rgba(0,0,0,0.35);padding:3px 7px;border-radius:20px;"><span style="width:5px;height:5px;border-radius:50%;background:${c.color};box-shadow:0 0 6px ${c.color};"></span>${it.pointCloudUrl ? '실측 데이터 미리보기' : '회전 미리보기'} · ${it.extra}</div></div>`;
  }
