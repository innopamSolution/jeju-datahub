...
export function thumbHtml(it, catMap, gallery) {
  ...
  if (realUrl) {
    ...
    const H = 230;
    return `<div style="position:relative;height:${H}px;background:radial-gradient(circle at 50% 40%, #14202e, #060a10);"><canvas class="sams-tt sams-tt-real" data-url="${realUrl}" data-kind="${kind}" width="320" height="${H}" style="width:100%;height:${H}px;display:block;"></canvas><div style="position:absolute;left:9px;bottom:8px;display:flex;align-items:center;gap:5px;font-size:10px;color:rgba(255,255,255,0.8);background:rgba(0,0,0,0.35);padding:3px 7px;border-radius:20px;"><span style="width:5px;height:5px;border-radius:50%;background:${c.color};box-shadow:0 0 6px ${c.color};"></span>실측 데이터 미리보기 · ${it.extra}</div></div>`;
  }
  ...
  if (it.cat === 'pano' && it.panoImages && it.panoImages.length) {
    ...
    const expandBtn = gallery ? `<button data-act="pano-large" title="크게 보기" style="position:absolute;right:8px;top:8px;width:26px;height:26px;border-radius:7px;border:none;background:rgba(0,0,0,0.45);color:#fff;cursor:pointer;display:flex;align-items:center;justify-content:center;">${EXPAND_SVG}</button>` : '';
    return `<div style="position:relative;height:138px;overflow:hidden;background:#1a2230;"><div class="sams-gframe sams-gframe-real" data-images="${imagesAttr}" data-idx="0" style="position:absolute;inset:0;background-size:cover;background-position:center;background-image:url('${it.panoImages[0]}');"></div><div style="position:absolute;left:10px;top:9px;display:flex;align-items:center;gap:5px;font-size:11px;font-weight:700;color:rgba(255,255,255,0.95);text-shadow:0 1px 3px rgba(0,0,0,0.6);"><span style="width:5px;height:5px;border-radius:50%;background:${c.color};box-shadow:0 0 6px ${c.color};"></span>파노라마 그룹</div>${expandBtn}${gallery ? `<button data-act="img-prev" style="position:absolute;left:8px;top:50%;transform:translateY(-50%);width:28px;height:28px;border-radius:50%;border:none;background:rgba(0,0,0,0.45);color:#fff;cursor:pointer;display:flex;align-items:center;justify-content:center;font-size:18px;line-height:1;">‹</button><button data-act="img-next" style="position:absolute;right:8px;top:50%;transform:translateY(-50%);width:28px;height:28px;border-radius:50%;border:none;background:rgba(0,0,0,0.45);color:#fff;cursor:pointer;display:flex;align-items:center;justify-content:center;font-size:18px;line-height:1;">›</button>` : ''}<span class="sams-gbadge" style="position:absolute;right:10px;bottom:9px;font-size:10px;color:#fff;background:rgba(0,0,0,0.5);padding:3px 8px;border-radius:20px;">1 / ${it.panoImages.length}</span></div>`;
  }
